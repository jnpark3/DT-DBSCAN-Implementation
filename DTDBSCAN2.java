...
public class DTDBSCAN2 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        runClustering(points, N, 30, 10);                                           // Run DTDBSCAN Clustering algorithm

        f.close();
    }

    public static void runClustering(Point[] inputPoints, int N, int EPSILON, int MINPTS)
            throws Exception {                                                      // ** DTDBSCAN Clustering Algorithm **

        long start, finish;
        start = System.currentTimeMillis();                                         // Measure Run Time Start
        ...
        if (inputPoints.length == 0) {                                              // Resolve case where input points set is empty
            return;
        }                                                                           

        xMin = xMax = inputPoints[0].x;                                             // Determine min/max x/y coordinates
        ...
        int gridSpaces = (int) Math.pow(4, (int) (Math.log(N) / Math.log(4)) + 1);  // No. of grid spaces in square; O(N) value
        int gridSize = (int) Math.sqrt(gridSpaces);                                 // No. of grid spaces in each row
        double width = Math.max((yMax - yMin), (xMax - xMin)) / gridSize;           // Width of grid that encloses inputPoints

        ArrayList<ArrayList<ArrayList<Integer>>> grid = new ArrayList<ArrayList<ArrayList<Integer>>>(); // Declare variable for grid

        for (int i = 0; i < gridSize; i++){                                         // Assign an ArrayList<Integer> for each grid space
        ...
        for (int i = 0; i < N; i++) {                                               // Assign each point from inputPoints to its grid space
        ...
        int[] morton = morton_order(gridSpaces, gridSize);                          // Generate Morton ordering curve for given grid dimensions
        Point[] points = new Point[N];                                              // Declare new Point array to store re-ordered inputPoints set
        int n = 0;                                                                  

        for (int i = 0; i < gridSpaces; i++) {                                      // Searches grid spaces in morton order and adds points in each grid
        ...
        //      - For each point 
        //      - Merges the triangulations in the grid spaces in a quad-tree order
        ...
        clusterOut(points, clusters, N, "clusterDTDBSCAN2.out");
    }
    ...
    public static boolean inside(Point a, Point b, Point c, Point d) { // Check if point a is in circumcircle of
        ...
        if (a.x == b.x && a.y == b.y)
            return false; // Resolve cases were two points are same
        if (a.x == c.x && a.y == c.y)
            return false;
        if (a.x == d.x && a.y == d.y)
            return false;
